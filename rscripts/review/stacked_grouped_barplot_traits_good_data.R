...
# Subset to only common categories
tra<-c("Biogeography", "Genome", "Habitat", "Pollination", "Reproduction", "Vegetative")
df2<-df2[df2$trait_level_2 %in% tra, ]
...
#number of studies meeting criteria (COMMON CATS ONLY)
unique(df3$study)
...
#no filtering
df2 <- df2 %>% group_by(trait_level_2, div_inc, .drop=FALSE) %>% summarize(count=n()) %>%   mutate(perc = count/sum(count)) %>% ungroup()  %>% complete(trait_level_2, div_inc, fill = list(count = 0))

#relaxed filtering
df3 <- df3 %>% group_by(trait_level_2, div_inc, .drop=FALSE) %>%
  summarize(count=n()) %>%
  mutate(perc = count/sum(count)) %>%
  ungroup()  %>%
  complete(trait_level_2, div_inc, fill = list(count = 0))

#fix NA issue
#df3$perc[is.na(df3$perc)]<-0

#strict filtering
df4 <- df4 %>% group_by(trait_level_2, div_inc, .drop=FALSE) %>% summarize(count=n()) %>%   mutate(perc = count/sum(count)) %>% ungroup()  %>% complete(trait_level_2, div_inc, fill = list(count = 0))
...
#add column with filtering label
df_all$filter<-c(rep("none",length(rownames(df2))),
                 rep("relaxed",length(rownames(df3))),
                 rep("strict",length(rownames(df4))))

#group by filter, calculate position for labels in plot
df_all_2<-df_all %>%
  group_by(trait_level_2, filter) %>%
...
ggsave("figures/review/stacked_grouped_barplot_traits_filtering.png",
       width = 20,
       height = 10,
       units = 'cm')
