...
my_theme = theme(
  ...
  axis.title.y = element_text(angle=90, size = 14, hjust = -0.5 , margin = margin(
    t = 0,
    r = 10,
    b = 0,
    l = 20
...
for (i in 1:length(unique(df$study))) {
  ...
  for (j in 1:length(unique(temp_df$model_no))) {
    ...
    if (temp_mod_df$div_rate[which.max(temp_mod_df$div_rate)] > 0 &&
        temp_mod_df$div_rate[which.min(temp_mod_df$div_rate)] > 100) {
...
p1 <- ggplot(df3, aes(
  ...
  annotate(geom="text", x=0, y=100, label="(a)",size=5) +
  coord_cartesian(ylim = c(0.001,2.5),
                  xlim = c(1, 3),
                  clip = 'off')
...
p2 <- ggplot(df4, aes(
  ...
  theme(legend.position = "none",
        axis.title.x = element_text(vjust = 0.3),
        axis.text.x = element_text(angle=90)) +
        scale_y_continuous(name = "Ratio between maximum and minimum net diversification rates") +
  scale_x_discrete(name = "Trait level 2") +
  scale_fill_viridis(discrete = TRUE, alpha = 0.3) +
  scale_color_viridis(discrete = TRUE, alpha = 0.8) +
  annotate(geom="text", x=-0.8, y=100, label="(b)",size=5) +
  coord_cartesian(ylim = c(0.001,2.5),
                  xlim = c(1, 9),
                  clip = 'off')
...
ggsave("figures/violins_reldiffs_all.png",
       width = 30,
       height = 30,
       units = 'cm')
