...
agg_st_mod<-aggregate(df$div_inc,by=list(df$study,df$model_no,df$trait_level_2),FUN=sum)
#view(agg_st_mod)
...
mc<-data.frame(c("Fire","Soil","Symbiosis","Fire","Soil","Symbiosis","Combination","Defense","Habitat"),
      c("1","1","1","0","0","0","0","0","1"),
      c(0,0,0,0,0,0,0,0,0))
...
ggsave(
  "figures/stacked_barplot_traits_level2_hidden.png",
  width = 20,
  height = 12,
  units = 'cm'
)
